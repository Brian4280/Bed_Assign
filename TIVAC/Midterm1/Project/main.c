...
//Similar to UARTprintf, only used for
//debugging.
...
//Initialize UART0 (Only for debugging purposes)
...
/*
 Only used for debugging, gets text from ESP AT commands and displays it
 onto the UART0 terminal so that I can see what each command results in.
 */
...
void Timer1IntHandler(void)
{
    ...
    lux = lux / 10;
    char getCmd[200] = "GET /update?key=DN72F6LKT2GVW8RD&field1=150&headers=falseHTTP/1.1\nHostapi.thingspeak.com\nConnection:close\Accept*\*\r\n\r\n";
    ...
    SysCtlDelay (13333333);
    UARTprintf ("AT+CIPSEND=4,%d\r\n", strlen(getCmd));  //command the ESP8266 to allow sending of information
    SysCtlDelay(13333333);
    UARTprintf("GET /update?key=DN72F6LKT2GVW8RD&field1=%d&headers=falseHTTP/1.1\nHostapi.thingspeak.com\nConnection:close\Accept*\*\r\n\r\n", lux);
    SysCtlDelay(10000);
...
int main(void)
{
    ...
    //ConfigureUART0();  //used for debugging purposes only
    ConfigureUART1();

    /*   used for debugging, displays received text from ESP and displays to console
